...
ob_start();
?>
<h1 class="h4 mb-3 module-title"><i class="bi bi-shield-check me-2"></i>Integração Fiscal (SEFAZ)</h1>
...
<input type="text" class="form-control mb-3" placeholder="Filtrar ao digitar..." data-filter-input="#tabela-fiscal">
<table class="table table-striped" id="tabela-fiscal" data-filter-table>
    <thead><tr><th>Documento</th><th>Tipo</th><th>Status</th></tr></thead>
    <tbody>
    <?php foreach ($documentos as $d): ?>
        <tr>
            <td><?= htmlspecialchars((string) $d['numero'], ENT_QUOTES, 'UTF-8') ?></td>
            <td><?= htmlspecialchars((string) $d['tipo'], ENT_QUOTES, 'UTF-8') ?></td>
